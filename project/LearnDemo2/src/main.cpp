...
void cursor_pos_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	if(flagDown)
	{
		...
		// cameraPos.x = 0;
		// cameraPos.y = 3.0f*sinf(lastdeltaY*0.1f);
		// cameraPos.z = 3.0f*cosf(lastdeltaY*0.1f);
		// cameraUp = glm::normalize(glm::cross(cameraPos,  glm::vec3(1.0f, 0.0f,  0.0f)));


		cameraPos.x = 3.0f*cosf(lastdeltaY*sp)*cosf(lastdeltaX*sp);
		cameraPos.y = 3.0f*sinf(lastdeltaY*sp);
		cameraPos.z = 3.0f*cosf(lastdeltaY*sp)*sinf(lastdeltaX*sp);
		// cameraUp = glm::normalize(glm::cross(cameraPos,  glm::vec3(1.0f, 0.0f,  0.0f)));

		// cameraPos = glm::normalize(cameraPos);

		glm::vec3 up = glm::vec3(0.0f, 1.0f, 0.0f); 
		glm::vec3 cameraRight = glm::normalize(glm::cross(up, cameraPos));
...
sp = 0.05f;
	while (!glfwWindowShouldClose(window))
	{
		...
		float ppx = glm::radians(lastdeltaX*sp);
		float ppy = glm::radians(lastdeltaY*sp);
		
		ImGui::SliderFloat("lastdeltaX", &ppx, -10.0, 10.0);
		ImGui::SliderFloat("lastdeltaY", &ppy, -10.0, 10.0);
		ImGui::End();
		ImGui::Render();
