...
void threeD_putline(Point a,Point b){
	...
	if(a.y<=0||b.y<=0||a.y>=28||b.y>=18) return;
	// setlinewidth(1);
...
void putpointxyz(int x, int y,int z,color_t color){
	// putpixel(x/y*800*camerasize+800+xl,500-z/y*800*camerasize+yl,color);
}
class threeD_object{
	public:
		Point point[2000];
...
void threeD_putobj(threeD_object a){
	for(int i=0;i<a.usepoint;i++){
		for(int j=0;j<a.usepoint;j++){
			threeD_putline(a.point[i],a.point[j]);
		}
	}
}
void threeD_putobj(threeD_object a);
